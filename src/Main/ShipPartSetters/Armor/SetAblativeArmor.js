...
function SetAblativeArmor(props) {
  ...
  const { acBonus, tempHP, tlPenalty, turnDistance, bpCost, maxHP } = Tables.getAblativeArmorData(ablativeArmorId, size);
  ...
  const createDropdownItem = (armor, idx) => {
    if (armor.includes("ablative")) return `${armor} (+${Tables.getAblativeArmorData(armor, size).tempHP} THP)`;
    if (armor.includes("interposed")) return `${armor} (+${Tables.getAblativeArmorData(armor, size).tempHP} THP)`;
  }
