...
import frames from "./CustomRefs/frames.json";
...
function SetFrame(props: SetterProps) {
  const { customShipParts, ship, activeSources } = useContext(CustomShipContext);
  const [specialName, setSpecialName] = useState<string | null>(null);
  ...
  const handleFrameIdChange = (ev: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    <>
      ...
      {specialAbility &&
        <fieldset>
          ...
          <div><strong>{specialName}</strong> {specialAbility[String(specialName)]}</div>
        </fieldset>
      }
