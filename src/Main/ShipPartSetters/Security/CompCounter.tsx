...
function CompCounter(props: SetterProps) {
  ...
  const calculateTotalBpCost = (): number => {
    let total = 0;
    Object.keys(counters).forEach((key) => {
      // const counterKey: any = counters[key]
  ...
  const checkboxRenders = (): Array<JSX.Element> => {
  ...
  const renderShockGridRanks = (tier: string, idx: number) => {
    const tierNum = Number(tier)
    const shockData = Tables.getComputerShockGridData(tier, computerTier)

    const tierWithinMaxSlots = (tierNum <= slotsAllowed);
    ...
    const tierWithinValidRemainder = tierNum <= remainingSlotsNotUsedByShocks
